...
StatusController.prototype.show = function(req, res) {
  ...
  switch(option) {
  case 'getListMasternodes':
    this.getMasternodeList(function(err, result) {
      if (err) {
  ...
  case 'getMasternodeList':
    ...
    break;
  case 'getGovObjectList':
    this.getGovObjectList(function(err, result) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
      res.jsonp(result);
...
StatusController.prototype.getMasternodeList = function(callback) {
  this.node.services.bitcoind.getMNList(function(err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.getListMasternodes = function(callback) {
  this.node.services.bitcoind.getListMasternodes(function(err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, {
      masternodesByRank: info
    });
  });
};
StatusController.prototype.getGovObjectList = function(callback) {
  var options = {
